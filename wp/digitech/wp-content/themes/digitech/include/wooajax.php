...
add_action('wp_head','digitech_woo_ajaxurl');
function digitech_woo_ajaxurl() {
?>
<?php
...
function product_quickview() {
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($digitech_opt['quickview_link_text']); ?></a>
			...
			<?php if( function_exists('digitech_blog_sharing') ) { ?>
				<div class="social-sharing"><?php digitech_product_sharing(); ?></div>
			<?php } ?>
		</div>
	</div>
	<?php
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'digitech'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'digitech');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'digitech');?></a>
	</div>
	<?php
	die();
}
