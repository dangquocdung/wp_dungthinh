...
function studylms_get_events( $type, $number = -1 ) {
    switch ( $type ) {
        case 'most_recent' : 
           $args = array( 
...
if ( !function_exists('studylms_event_content_class') ) {
    function studylms_event_content_class( $class ) {
        $page = 'archive';
        if ( is_singular( 'tribe_events' ) ) {
            $page = 'single';
        }
        if( studylms_get_config('event_'.$page.'_fullwidth') ) {
            return 'container-fluid';
        }
        return $class;
    }
}
add_filter( 'studylms_event_content_class', 'studylms_event_content_class' );

// get layout event
if ( !function_exists('studylms_get_event_layout_configs') ) {
    function studylms_get_event_layout_configs() {
        $page = 'archive';
        if ( is_singular( 'tribe_events' ) ) {
            $page = 'single';
        }
        $left = studylms_get_config('event_'.$page.'_left_sidebar');
        $right = studylms_get_config('event_'.$page.'_right_sidebar');

        switch ( studylms_get_config('event_'.$page.'_layout') ) {
            case 'left-main':
...
function studylms_tribe_events_event_schedule_start( $event = null, $before = '', $after = '' ) {
    if ( is_null( $event ) ) {
        global $post;
        $event = $post;
    }
...
function studylms_tribe_events_event_schedule_finish( $event = null, $before = '', $after = '' ) {
    if ( is_null( $event ) ) {
        global $post;
        $event = $post;
    }
