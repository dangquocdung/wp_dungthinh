( function( $ ) {
	...
	function wpc_instance(param){
		return typeof wp.customize.instance(param)!='undefined' ? wp.customize.instance(param).get() : '';
	}

	function fixing_color_value($value){
		var $val = $value+"";
		if( $val.length>0 && $val.indexOf("(")>-1 && $val.indexOf(")")<0 ){
			$val += ")";
		}
		return $val;
	}
	...
	function modify_vars_less(){
		...
		for( var i=0; i<$variables.length; i++ ){
			var modified_value = wpc_instance($variables[i]);
			if( modified_value!='' ){
				modified_value = fixing_color_value(modified_value);
	...
	for( var i=0; i<$font_size_vars.length; i++ ){
		wp.customize( $font_size_vars[i], function(value){
			value.on('bind', function(newval){ modify_vars_less(); } );
		});
	}
	...
	jQuery(window).load(function(){
		...
		var site_layout = wpc_instance('general-layout');
		if( !$('body').hasClass(site_layout) ){
			$('body').removeClass('full boxed').addClass(site_layout);
		}

		if( site_layout=='boxed' ){
			var site_margin_top = wpc_instance('general-top-space');
			var site_margin_bottom = wpc_instance('general-bottom-space');
		}

		$('body').css('background-image', 'url('+wpc_instance('general-bg-image')+')');
		$('body').css('background-repeat', wpc_instance('general-bg-repeat'));
		$('body').css('background-position', wpc_instance('general-bg-position'));
		$('body').css('background-attachment', wpc_instance('general-bg-attach'));
	...
	wp.customize( 'logo', function(value){
		value.on('bind', function(newval){
			$('.main-menu .logo').html('<a href="#"><img src="'+newval+'" alt="'.esc_attr__('spare','onetheme').'" class="normal" /></a>');
		} );
	});
