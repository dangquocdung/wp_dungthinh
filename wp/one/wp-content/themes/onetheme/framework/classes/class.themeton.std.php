...
class TTStd{
    ...
    public static function get_mod($name, $default = false){
        ...
        if( !$mod_value )
            return '';
        return !empty($mod_value) ? TTStd::clear_xss($mod_value) : '';
    }
    ...
    public static function get_option_bg_value($value){
        $bg = TTStd::get_option_bg(TTStd::get_mod($value));
        $result = TTStd::extract_bg_values($bg);
        return $result;
    }
    ...
    public static function get_meta_bg_value($value){
        $val = TTStd::getmeta($value);
        $val = str_replace("$", "|", $val);
        $bg = TTStd::get_option_bg($val);
        $result = TTStd::extract_bg_values($bg);
        return $result;
    }
    ...
    public static function google_fonts(){
        global $onetheme_google_fonts;
        if( !empty($onetheme_google_fonts) )
            return $onetheme_google_fonts;
        
        include_once get_template_directory()."/framework/functions/google-fonts.php";
        $onetheme_google_fonts = get_google_webfonts();
    ...
    public static function fs_get_contents_array($path){
        $file_content = TTStd::fs_get_contents($path);
        return explode("\n", $file_content );
    }
