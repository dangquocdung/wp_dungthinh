...
function rdn_top_title() {
    if ( is_active_widget( false, false, 'rdn_top_title', true ) || is_singular( 'rdn-slider' )) {
...
add_action('wp_enqueue_scripts', 'rdn_top_title', 20, 1);


/**
 * Adds Foo_Widget widget.
 */
class rdn_top_title extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array(
			'classname' => 'widget_rdn_top_title', 
			'description' => __('Display top title with background.'),
			'panels_groups' => array('cordon_builder'),
			'panels_icon' => 'dashicons dashicons-vault'
		);
        parent::__construct('rdn_top_title', __('Top Title Block'), $widget_ops);
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $top_title = isset( $instance['top_title'] ) ? $instance['top_title']  : '';
        ...
        <!--PAGE HEAD SLIDER START-->
        <div class="<?php if  ($s_format != 'image') { echo 'home-video-box'; } else {echo 'bg-on';}?>">
            <div class="home-slider page-head-slider ani-slider slider" data-slick='{"autoplaySpeed": 8000}'>
                    ...
                    <div class="slider-box container-fluid">
                        <div class="slider-content left-box-slider">
                            <div class="slider-hidden">
                                <h3 class="slider-title" data-animation="fadeInUp" data-delay="0.8s"><?php echo wp_kses_post( $top_title );  ?></h3>
                            </div><!--/.slider-hidden-->
...
?>
    <p>
     <label for="<?php echo $this->get_field_id('top_title'); ?>">
        <?php _e('The Title:', 'cordon_plg'); ?>
      </label>
      <input class="widefat" id="<?php echo $this->get_field_id('top_title'); ?>" name="<?php echo $this->get_field_name('top_title'); ?>" type="text" value="<?php echo $top_title ?>">
      <br/>Insert the top title here<br/>
      You can use &lt;span&gt;&lt;/span&gt; tag to give the title the theme style color.
    </p>
    
    <p>  
      <label for="<?php echo $this->get_field_id('text'); ?>">
        <?php _e('The Text:', 'cordon_plg'); ?>
      </label>
   ...
   <p>
      <label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php _e( 'Image Background:' ); ?></label>
...
function register_rdn_top_title_widget() {
	global $pagenow;
	if ($pagenow != 'widgets.php') {
	register_widget( 'rdn_top_title' );
	}
}
add_action( 'widgets_init', 'register_rdn_top_title_widget' );
