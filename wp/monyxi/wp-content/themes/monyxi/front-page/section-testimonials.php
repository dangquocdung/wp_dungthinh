<div class="front_page_section front_page_section_testimonials<?php
			$monyxi_scheme = monyxi_get_theme_option('front_page_testimonials_scheme');
			if (!monyxi_is_inherit($monyxi_scheme)) echo ' scheme_'.esc_attr($monyxi_scheme);
			echo ' front_page_section_paddings_'.esc_attr(monyxi_get_theme_option('front_page_testimonials_paddings'));
		?>"<?php
		$monyxi_css = '';
		$monyxi_bg_image = monyxi_get_theme_option('front_page_testimonials_bg_image');
...
?>><?php
	// Add anchor
	$monyxi_anchor_icon = monyxi_get_theme_option('front_page_testimonials_anchor_icon');	
	$monyxi_anchor_text = monyxi_get_theme_option('front_page_testimonials_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_testimonials_inner<?php
			if (monyxi_get_theme_option('front_page_testimonials_fullheight'))
				echo ' monyxi-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$monyxi_css = '';
			$monyxi_bg_mask = monyxi_get_theme_option('front_page_testimonials_bg_mask');
			$monyxi_bg_color = monyxi_get_theme_option('front_page_testimonials_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$monyxi_caption = monyxi_get_theme_option('front_page_testimonials_caption');
			if (!empty($monyxi_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo !empty($monyxi_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($monyxi_caption, 'monyxi_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$monyxi_description = monyxi_get_theme_option('front_page_testimonials_description');
			if (!empty($monyxi_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo !empty($monyxi_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($monyxi_description), 'monyxi_kses_content'); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_testimonials_output"><?php 
				if (is_active_sidebar('front_page_testimonials_widgets')) {
					...
					if (!monyxi_exists_trx_addons())
						...
						monyxi_customizer_need_widgets_message('front_page_testimonials_caption', 'ThemeREX Addons - Testimonials');
				}
			?></div>
		</div>
	</div>
