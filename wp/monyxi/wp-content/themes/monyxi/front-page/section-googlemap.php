<div class="front_page_section front_page_section_googlemap<?php
			$monyxi_scheme = monyxi_get_theme_option('front_page_googlemap_scheme');
			if (!monyxi_is_inherit($monyxi_scheme)) echo ' scheme_'.esc_attr($monyxi_scheme);
			echo ' front_page_section_paddings_'.esc_attr(monyxi_get_theme_option('front_page_googlemap_paddings'));
		?>"<?php
		$monyxi_css = '';
		$monyxi_bg_image = monyxi_get_theme_option('front_page_googlemap_bg_image');
...
?>><?php
	// Add anchor
	$monyxi_anchor_icon = monyxi_get_theme_option('front_page_googlemap_anchor_icon');	
	$monyxi_anchor_text = monyxi_get_theme_option('front_page_googlemap_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_googlemap_inner<?php
			if (monyxi_get_theme_option('front_page_googlemap_fullheight'))
				echo ' monyxi-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$monyxi_css = '';
			$monyxi_bg_mask = monyxi_get_theme_option('front_page_googlemap_bg_mask');
			$monyxi_bg_color = monyxi_get_theme_option('front_page_googlemap_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_googlemap_content_wrap<?php
			$monyxi_layout = monyxi_get_theme_option('front_page_googlemap_layout');
			if ($monyxi_layout != 'fullwidth')
				echo ' content_wrap';
		?>">
			<?php
			// Content wrap with title and description
			$monyxi_caption = monyxi_get_theme_option('front_page_googlemap_caption');
			$monyxi_description = monyxi_get_theme_option('front_page_googlemap_description');
			if (!empty($monyxi_caption) || !empty($monyxi_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($monyxi_layout == 'fullwidth') {
					...
					if (!empty($monyxi_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><h2 class="front_page_section_caption front_page_section_googlemap_caption front_page_block_<?php echo !empty($monyxi_caption) ? 'filled' : 'empty'; ?>"><?php
					...
					if (!empty($monyxi_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><div class="front_page_section_description front_page_section_googlemap_description front_page_block_<?php echo !empty($monyxi_description) ? 'filled' : 'empty'; ?>"><?php
			...
			// Content (text)
			$monyxi_content = monyxi_get_theme_option('front_page_googlemap_content');
			if (!empty($monyxi_content) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($monyxi_layout == 'columns') {
					?><div class="front_page_section_columns front_page_section_googlemap_columns columns_wrap">
						<div class="column-1_3">
					<?php
				} else if ($monyxi_layout == 'fullwidth') {
					?><div class="content_wrap"><?php
				}
	
				?><div class="front_page_section_content front_page_section_googlemap_content front_page_block_<?php echo !empty($monyxi_content) ? 'filled' : 'empty'; ?>"><?php
			...
			?><div class="front_page_section_output front_page_section_googlemap_output"><?php 
				if (is_active_sidebar('front_page_googlemap_widgets')) {
					...
					if (!monyxi_exists_trx_addons())
						...
						monyxi_customizer_need_widgets_message('front_page_googlemap_caption', 'ThemeREX Addons - Google map');
				}
			?></div><?php
