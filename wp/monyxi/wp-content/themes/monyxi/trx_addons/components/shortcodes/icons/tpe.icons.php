...
#><div id="{{ id }}" class="sc_icons sc_icons_{{ settings.type }} sc_icons_size_{{ settings.size }} sc_align_{{ settings.align }}">
	...
	_.each(settings.icons, function(item) {
		...
		#><div class="sc_icons_item<# if (item.link.url != '') print(' sc_icons_item_linked'); #>"><#
			...
			if (item.image.url != '') {
				#><div class="sc_icons_image"><img src="{{ item.image.url }}" alt="<?php esc_attr_e('Icon', 'monyxi'); ?>"></div>
                <div class="header-line">
                <#
                var icon = trx_addons_is_off( item.icon );
                ...
                var img = '', svg = '';
					...
					#>><#
						if (svg != '') {
							...
							#><img class="sc_icon_as_image" src="{{ img }}" alt="<?php esc_attr_e('Icon', 'monyxi'); ?>"><#
						} else {
