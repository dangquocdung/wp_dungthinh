...
?>><?php
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"><?php
            ...
            if (!empty($icon)) {
                ...
                if (!empty($svg)) {
                    ...
                    ?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'monyxi'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
                } else {
            ...
            if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', monyxi_get_thumb_size('big'), 'icons-extra'));
				if (!empty($item['image'])) {
					$attr = trx_addons_getimagesize($item['image']);
					?><div class="sc_icons_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'monyxi'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			}
			...
			if (!empty($item['title'])) { ?>
                ...
                <span class="num">0<?php echo esc_html($num); ?></span>
        ...
        if (!empty($item['link']) && !empty($item['link_text'])) {
            ?><a class="sc_button" href="<?php echo esc_url($item['link']); ?>"
            <?php
            if (!empty($item['new_window']) || !empty($item['link_extra']['is_external'])) echo ' target="_blank"';
            ?>><?php
