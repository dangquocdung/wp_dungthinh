...
if (!function_exists('monyxi_customizer_get_css')) {

	function monyxi_customizer_get_css($args=array()) {
		...
		if ($fonts) {
			...
			$fonts = monyxi_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('monyxi_filter_get_css', $rez, array('fonts' => $fonts));
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ($vars) {
			...
			// Make theme-specific fonts rules
			$vars = monyxi_customizer_add_theme_vars($vars);
			extract($vars);
...
CSS;
			$rez = apply_filters('monyxi_filter_get_css', $rez, array('vars' => $vars));
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($scheme) ? array_keys(monyxi_get_sorted_schemes()) : array($scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $s) {
					// Prepare colors
					if (empty($scheme)) $colors = monyxi_get_scheme_colors($s);
...
.lp-list-table tfoot th, .lp-list-table tfoot td {
	background-color: {$colors['alter_bg_hover_02']} !important;
}
hr {
...
.wp-caption-overlay .wp-caption .wp-caption-dd {
	color: {$colors['text_dark']};
	background-color: {$colors['text_hover2_09']};
}
ul > li:before {
	color: {$colors['text_link']};
}
...
.sc_layouts_row_type_narrow .sc_layouts_item a:not(.sc_button):not(.button):hover .sc_layouts_iconed_text_details *,
.scheme_self.sc_layouts_row_type_narrow .sc_layouts_item a:not(.sc_button):not(.button):hover .sc_layouts_iconed_text_details * {
	color: {$colors['alter_dark']};
}
...
.sc_layouts_row_type_compact .widget_contacts .contacts_info > span:hover:before,
.sc_layouts_row_type_compact .widget_contacts .contacts_info > a:hover:before {
...
.monyxi_tabs .monyxi_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.monyxi_tabs .monyxi_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .monyxi_tabs .monyxi_tabs_titles li.ui-state-active a {
...
.footer_widgets_wrap .widget_calendar td#today {
	color: {$colors['inverse_link']} !important;
}
...
.mfp-bg,
.elementor-lightbox {
	background-color: rgba(27,26,19,0.9);
}
...
.post_layout_excerpt.sticky .post_header .post_categories a:hover {
	background-color: {$colors['extra_hover']};
	color: {$colors['inverse_link']};
}
.post_layout_excerpt.sticky .post_meta .post_date a:before, 
.post_layout_excerpt.sticky .post_meta a.post_meta_item:before, 
.post_layout_excerpt.sticky .post_meta .vc_inline-link:before {
	color: {$colors['text_hover2']};
} 
...
.post_item_single .post_header .post_meta .post_date a:before, 
.post_item_single .post_header .post_meta a.post_meta_item:before, 
.post_item_single .post_header .post_meta .vc_inline-link:before {
	color: {$colors['text_dark']};
}
...
.extra-table .buy {
 ...
 .course-permalink:hover .course-title {
	 color: {$colors['text_hover']};
 }
...
CSS;
					...
					$rez = apply_filters('monyxi_filter_get_css', $rez, array('colors' => $colors, 'scheme' => $s));
		...
		return apply_filters( 'monyxi_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>