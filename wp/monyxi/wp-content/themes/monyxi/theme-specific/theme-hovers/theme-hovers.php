...
if (!function_exists('monyxi_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'monyxi_hovers_theme_setup3', 3 );
	function monyxi_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		monyxi_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'monyxi'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'monyxi') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'monyxi'),
			...
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'monyxi'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'monyxi') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
					'section' => esc_html__('Content', 'monyxi')
				),
				"options" => monyxi_get_list_hovers(),
				"type" => "select"
			) )
		);
	}
...
if (!function_exists('monyxi_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'monyxi_hovers_theme_setup9', 9 );
	function monyxi_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'monyxi_hovers_frontend_scripts', 1010 );
		add_filter( 'monyxi_filter_localize_script','monyxi_hovers_localize_script' );
		add_filter( 'monyxi_filter_merge_scripts',	'monyxi_hovers_merge_scripts' );
		add_filter( 'monyxi_filter_merge_styles',	'monyxi_hovers_merge_styles' );
		add_filter( 'monyxi_filter_merge_styles_responsive', 'monyxi_hovers_merge_styles_responsive' );
		add_filter( 'monyxi_filter_get_css', 		'monyxi_hovers_get_css', 10, 2 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'monyxi_hovers_frontend_scripts' ) ) {
	
	function monyxi_hovers_frontend_scripts() {
		if ( monyxi_is_on(monyxi_get_theme_option('debug_mode')) && ($monyxi_url = monyxi_get_file_url('theme-specific/theme-hovers/theme-hovers.js')) != '' )
			wp_enqueue_script( 'monyxi-hovers', $monyxi_url, array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('monyxi_hovers_merge_styles')) {
	
	function monyxi_hovers_merge_styles($list) {
...
if (!function_exists('monyxi_hovers_merge_styles_responsive')) {
	
	function monyxi_hovers_merge_styles_responsive($list) {
...
if (!function_exists('monyxi_hovers_localize_script')) {
	
	function monyxi_hovers_localize_script($arr) {
		$arr['button_hover'] = monyxi_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects to the single js
if (!function_exists('monyxi_hovers_merge_scripts')) {
	
	function monyxi_hovers_merge_scripts($list) {
...
if ( !function_exists('monyxi_hovers_add_icons') ) {
	function monyxi_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					monyxi_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'monyxi') : esc_html__('Buy now', 'monyxi')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'monyxi');
						else
							esc_html_e('Details', 'monyxi');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php echo !empty($post_link) ? esc_url($post_link) : esc_attr($no_link); ?>" aria-hidden="true" class="icon-01-link"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'monyxi_hovers_get_css' ) ) {
	
	function monyxi_hovers_get_css($css, $args) {
