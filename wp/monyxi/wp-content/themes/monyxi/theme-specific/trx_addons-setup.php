...
------------------------------------------------------------------------------- */
if (!function_exists('monyxi_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'monyxi_trx_addons_theme_specific_components');
	function monyxi_trx_addons_theme_specific_components($enable=false) {
		return MONYXI_THEME_FREE
					? false		// Free version
...
if (!function_exists('monyxi_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'monyxi_trx_addons_theme_specific_setup1', 1 );
	function monyxi_trx_addons_theme_specific_setup1() {
		if (monyxi_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'monyxi_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'monyxi_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'monyxi_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'monyxi_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'monyxi_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'monyxi_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'monyxi_trx_addons_cv_enable' ) ) {
	
	function monyxi_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'monyxi_trx_addons_demo_enable' ) ) {
	
	function monyxi_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'monyxi_trx_addons_edd_themes_market_enable' ) ) {
	
	function monyxi_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'monyxi_trx_addons_api_list' ) ) {
	
	function monyxi_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'monyxi_trx_addons_cpt_list' ) ) {
	
	function monyxi_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'monyxi_trx_addons_sc_list' ) ) {
	
	function monyxi_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'monyxi_trx_addons_widgets_list' ) ) {
	
	function monyxi_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'monyxi_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'monyxi_trx_addons_menu_cache');
	function monyxi_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('monyxi_trx_addons_localize_script')) {
	...
	function monyxi_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = monyxi_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'monyxi_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'monyxi_trx_addons_sc_type', 10, 2);
	function monyxi_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ($sc == 'trx_sc_blogger') {
			$list = monyxi_array_merge($list, monyxi_get_list_blog_styles(false, 'sc'));
		}
...
if ( !function_exists( 'monyxi_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'monyxi_trx_addons_sc_prepare_atts', 10, 2);
	function monyxi_trx_addons_sc_prepare_atts($atts, $sc) {
		if ( $sc == 'trx_sc_blogger') {
			$list = monyxi_get_list_blog_styles(false, 'sc');
...
if ( !function_exists( 'monyxi_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'monyxi_trx_addons_sc_atts', 10, 2);
	function monyxi_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'monyxi_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'monyxi_trx_addons_sc_map', 10, 2);
	function monyxi_trx_addons_sc_map($params, $sc) {
		...
		if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
			...
			$params['params'][] = array(
					'param_name' => 'scheme',
					'heading' => esc_html__('Color scheme', 'monyxi'),
					'description' => wp_kses_data( __('Select color scheme to decorate this block', 'monyxi') ),
					'group' => esc_html__('Colors', 'monyxi'),
					'admin_label' => true,
					'value' => array_flip(monyxi_get_list_schemes(true)),
					'type' => 'dropdown'
				);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading' => esc_html__('Color style', 'monyxi'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'monyxi') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(monyxi_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
		...
		if ($sc == 'trx_sc_blogger') {
			...
			$param = array(
					'param_name' => 'hover',
					'heading' => esc_html__('Image hover', 'monyxi'),
					'description' => wp_kses_data( __('Select hover effect for the featured image', 'monyxi') ),
					'edit_field_class' => 'vc_col-sm-4',
					'value' => array_flip(monyxi_get_list_hovers(true)),
					'dependency' => array(
						'element' => 'type',
...
if ( !function_exists( 'monyxi_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'monyxi_trx_addons_sow_map', 10, 2);
	function monyxi_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'monyxi'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'monyxi') ),
				'options' => monyxi_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			monyxi_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			monyxi_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output from new params
if ( !function_exists( 'monyxi_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'monyxi_trx_addons_sc_output', 10, 4);
	function monyxi_trx_addons_sc_output($output, $sc, $atts, $content) {
		
		if (in_array($sc, array('trx_sc_action'))) {
			if (!empty($atts['scheme']) && !monyxi_is_inherit($atts['scheme']))
				$output = str_replace('class="sc_action ', 'class="sc_action scheme_'.esc_attr($atts['scheme']).' ', $output);
			if (!empty($atts['color_style']) && !monyxi_is_inherit($atts['color_style']) && $atts['color_style']!='default')
...
if ( !function_exists( 'monyxi_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'monyxi_trx_addons_sc_item_title_tag');
	function monyxi_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'monyxi_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'monyxi_trx_addons_sc_item_button_args', 10, 3);
	function monyxi_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'monyxi_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'monyxi_trx_addons_slider_title', 10, 2 );
	function monyxi_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'monyxi_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'monyxi_trx_addons_sc_googlemap_styles');
	function monyxi_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'monyxi');
		$list['extra'] = esc_html__('Extra', 'monyxi');
		return $list;
	}
}
...
if ( !function_exists( 'monyxi_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'monyxi_trx_addons_tiny_mce_style_formats');
	function monyxi_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'monyxi') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'monyxi'),
								'selector' => 'ul',
...
if ( !function_exists( 'monyxi_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'monyxi_filter_allow_override_header_image', 'monyxi_trx_addons_allow_override_header_image' );
	function monyxi_trx_addons_allow_override_header_image($allow) {
		return monyxi_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'monyxi_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'monyxi_trx_addons_thumb_size', 10, 2);
	function monyxi_trx_addons_thumb_size($thumb_size='', $type='') {
...
if (!function_exists('monyxi_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'monyxi_trx_addons_override_options_fields', 10, 2);
	function monyxi_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			if (!isset($mb['email'])) {
				$mb['email'] = array(
					"title" => esc_html__("E-mail",  'monyxi'),
					"desc" => wp_kses_data( __("Team member's email", 'monyxi') ),
					"std" => "",
					"details" => true,
					"type" => "text"
				);
...
if ( ! function_exists( 'monyxi_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'monyxi_filter_get_list_menu_hover' );
    function monyxi_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'monyxi_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'monyxi_filter_get_list_input_hover' );
    function monyxi_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
