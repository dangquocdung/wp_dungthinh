...
if ( !function_exists('monyxi_need_page_title') ) {
	function monyxi_need_page_title() {
		return !is_front_page() && apply_filters('monyxi_filter_need_page_title', true);
	}
}
...
if ( !function_exists('monyxi_show_layout') ) {
	function monyxi_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('monyxi_get_logo_image') ) {
	function monyxi_get_logo_image($type='') {
		$logo_image = '';
		if (monyxi_is_on(monyxi_get_theme_option('logo_retina_enabled')) && monyxi_get_retina_multiplier(2) > 1)
			$logo_image = monyxi_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = monyxi_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return monyxi_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('monyxi_get_header_video') ) {
	function monyxi_get_header_video() {
		$video = '';
		if (apply_filters('monyxi_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (monyxi_check_theme_option('header_video')) {
				$video = monyxi_get_theme_option('header_video');
...
if ( !function_exists('monyxi_show_post_meta') ) {
	function monyxi_show_post_meta($args=array()) {

		if (is_single() && monyxi_is_off(monyxi_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post date
					...
					$dt = apply_filters('monyxi_filter_get_post_date', monyxi_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( ($output = monyxi_get_post_counters($args['counters'])) != '')
						monyxi_show_layout($output);
					else if (!monyxi_exists_trx_addons()) {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php comments_link(); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'monyxi'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					monyxi_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'monyxi'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'monyxi' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				// Custom counter
				} else {
					do_action('monyxi_action_show_post_counter', $comp, get_the_ID());
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('monyxi_show_post_featured') ) {
	function monyxi_show_post_featured($args=array()) {
		$args = array_merge(array(
			'hover' => monyxi_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: monyxi_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? monyxi_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = monyxi_get_post_image();
				if (!empty($image)) 
					$image = monyxi_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . monyxi_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				?><div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"><?php
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 monyxi_is_on(monyxi_get_theme_option('seo_snippets'))
													 	? array('itemprop' => 'image')
														: '');
					if (monyxi_get_theme_setting('attachments_navigation')) {
				...
				?>><?php
					if (has_post_thumbnail() && $args['seo']) {
						$monyxi_attr = monyxi_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($monyxi_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($monyxi_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						monyxi_hovers_add_icons($args['hover'], array('no_links' => $args['no_links']));
					}
					...
					if (($output=monyxi_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						monyxi_show_layout($output);
					...
					if (!empty($image)) {
						...
						monyxi_hovers_add_icons($args['hover'], array('no_links' => $args['no_links'], 'image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = monyxi_get_post_video($post_content, false);
						if (empty($video))
							$video = monyxi_get_post_iframe($post_content, false);
						if (empty($video)) {
					...
					// Put audio over the thumb
						...
						$audio = monyxi_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = monyxi_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = monyxi_array_get_first($audios, false);
								}
							}
						}
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								...
								$media_author = monyxi_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = monyxi_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !monyxi_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php monyxi_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !monyxi_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php monyxi_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								monyxi_show_layout($audio); 
							?></div><?php
						}
					}
				}
...
if ( !function_exists('monyxi_get_no_image') ) {
	function monyxi_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = monyxi_get_theme_option( 'no_image' );
			if (empty($img)) $img = monyxi_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('monyxi_add_bg_in_post_nav') ) {
	function monyxi_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = monyxi_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), monyxi_get_thumb_size('med') );
				$img = $img[0];
			} else if (monyxi_get_theme_setting('allow_no_image'))
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), monyxi_get_thumb_size('med') );
				$img = $img[0];
			} else if (monyxi_get_theme_setting('allow_no_image'))
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'monyxi-main', $css );
	}
}

// Show related posts
if ( !function_exists('monyxi_show_related_posts') ) {
	function monyxi_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
//	Attention! Parameter 'suppress_filters' is damage WPML-queries!
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : monyxi_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'monyxi');
				?></h3>
				<div class="columns_wrap posts_container columns_padding_bottom">
					<?php
					while ( $query->have_posts() ) { $query->the_post();
						?><div class="column-1_<?php echo intval(max(1, min(4, $args['columns']))); ?>"><?php
							 get_template_part( apply_filters('monyxi_filter_get_template_part', 'templates/related-posts', $style), $style );
						?></div><?php
					}
					wp_reset_postdata();
					?>
...
if ( !function_exists('monyxi_show_portfolio_posts') ) {
	function monyxi_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? monyxi_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = monyxi_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = monyxi_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
		...
		while ( have_posts() ) { the_post(); 
			...
			$monyxi_part = $args['sticky'] && is_sticky() ? 'sticky' : ($style == 'gallery' ? 'portfolio-gallery' : $style);
...
// AJAX handler for the monyxi_ajax_get_posts action
if ( !function_exists( 'monyxi_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_monyxi_ajax_get_posts',			'monyxi_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_monyxi_ajax_get_posts',	'monyxi_ajax_get_posts_callback');
	function monyxi_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			monyxi_storage_set('blog_mode', 'blog');
			monyxi_storage_set('options_meta', get_post_meta($id, 'monyxi_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => monyxi_show_portfolio_posts(array(
							'cat' => intval(wp_unslash(monyxi_get_value_gpc('cat'))),
							'parent_cat' => intval(wp_unslash(monyxi_get_value_gpc('parent_cat'))),
							'page' => intval(wp_unslash(monyxi_get_value_gpc('page'))),
							'post_type' => trim(wp_unslash(monyxi_get_value_gpc('post_type'))),
							'taxonomy' => trim(wp_unslash(monyxi_get_value_gpc('taxonomy'))),
							'blog_style' => trim(wp_unslash(monyxi_get_value_gpc('blog_style'))),
							'echo' => false
							)
						)
		);
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'monyxi');
		}
...
// Show pagination
if ( !function_exists('monyxi_show_pagination') ) {
	function monyxi_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = monyxi_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'monyxi' ),
				'next_text' => esc_html__( '>', 'monyxi' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'monyxi' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'monyxi'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'monyxi') : esc_html__('Newest posts', 'monyxi') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'monyxi') : esc_html__('Older posts', 'monyxi'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
