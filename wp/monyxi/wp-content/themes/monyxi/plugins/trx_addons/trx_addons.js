...
(function() {
	...
	jQuery(document).on('action.add_googlemap_styles', monyxi_trx_addons_add_googlemap_styles);
	jQuery(document).on('action.init_hidden_elements', monyxi_trx_addons_init);
	
	// Add theme specific styles to the Google map
	function monyxi_trx_addons_add_googlemap_styles(e) {
	...
	function monyxi_trx_addons_init(e, container) {
		if (arguments.length < 2) var container = jQuery('body');
		if (container===undefined || container.length === undefined || container.length == 0) return;
		container.find('.sc_countdown_item canvas:not(.inited)').addClass('inited').attr('data-color', MONYXI_STORAGE['alter_link_color']);
	}
