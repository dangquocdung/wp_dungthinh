...
// Merge blogger specific scripts into single file
if ( !function_exists( 'trx_addons_sc_blogger_merge_scripts' ) ) {
    ...
    function trx_addons_sc_blogger_merge_scripts($list) {
        $list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'blogger/_blogger.js';
        return $list;
    }
}
...
if ( !function_exists( 'trx_addons_sc_blogger_load_scripts_front' ) ) {
    ...
    function trx_addons_sc_blogger_load_scripts_front() {
        if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
            wp_enqueue_script( 'trx_addons-sc_blogger', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'blogger/_blogger.js'), array('jquery'), null, true );
        }
    }
}
...
if ( !function_exists( 'trx_addons_sc_ajax_get_posts' ) ) {
    add_action('wp_ajax_trx_addons_ajax_get_posts',			'trx_addons_sc_ajax_get_posts');
    add_action('wp_ajax_nopriv_trx_addons_ajax_get_posts',	'trx_addons_sc_ajax_get_posts');
    function trx_addons_sc_ajax_get_posts() {
        if ( !wp_verify_nonce( trx_addons_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
            die();

        $response = apply_filters('trx_addons_sc_blogger_filter_ajax_get_posts', array('error'=>'', 'data' => ''));
