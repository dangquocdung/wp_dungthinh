...
function register_food_post_type() {
    ...
    $labels = array(
        'name'               => esc_html__( 'Foods', 'fundrize' ),
        'singular_name'      => esc_html__( 'Food Item', 'fundrize' ),
...
add_filter( 'post_updated_messages', 'food_updated_messages' );
/**
  * Food update messages.
*/
function food_updated_messages( $messages ) {
    $post             = get_post();
    ...
    $messages['food'] = array(
        ...
        1  => esc_html__( 'Food updated.', 'fundrize' ),
        2  => esc_html__( 'Custom field updated.', 'fundrize' ),
        3  => esc_html__( 'Custom field deleted.', 'fundrize' ),
        4  => esc_html__( 'Food updated.', 'fundrize' ),
        5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Food restored to revision from %s', 'fundrize' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
        6  => esc_html__( 'Food published.', 'fundrize' ),
        7  => esc_html__( 'Food saved.', 'fundrize' ),
        8  => esc_html__( 'Food submitted.', 'fundrize' ),
        9  => sprintf(
            esc_html__( 'Food scheduled for: <strong>%1$s</strong>.', 'fundrize' ),
            date_i18n( esc_html__( 'M j, Y @ G:i', 'fundrize' ), strtotime( $post->post_date ) )
        ),
        10 => esc_html__( 'Food draft updated.', 'fundrize' )
    );
    return $messages;
}

add_action( 'init', 'register_food_taxonomy' );
/**
  * Register food taxonomy
*/
function register_food_taxonomy() {
    $cat_slug = 'food_category';

    $labels = array(
        'name'                       => esc_html__( 'Food Categories', 'fundrize' ),
    ...
    register_taxonomy( 'food_category', 'food', $args );
}