...
$cls = $image_style;
$item_css = $text_css = $name_css = $name_cls = $position_css = $thumb_css = $name_pos_css = '';
...
if ( $name || $position ) {
	if ( $name ) {
	    $name_html .= sprintf(
	    '<h6 class="name %3$s" style="%2$s">%1$s</h6>',
	    $name, $name_css, $name_cls
	    );
	}
...
$text_html = sprintf(
	'<blockquote class="text" style="%2$s">
	<div class="quote-text">%1$s</div><div class="name-pos" style="%5$s">%3$s %4$s</div>
	</blockquote>',
