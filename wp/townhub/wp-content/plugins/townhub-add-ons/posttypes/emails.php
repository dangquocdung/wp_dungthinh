...
class Esb_Class_Emails
{
    ...
    public static function cthclaim_approved($claim_id = 0, $listing_id = 0, $user_id = 0)
    {
        if (is_numeric($claim_id) && (int) $claim_id > 0) {
            ...
            $subject = apply_filters('townhub-addons-claim-approved-subject', __('Claim listing approved', 'townhub-add-ons'));
            $body    = apply_filters('townhub-addons-claim-approved-body', sprintf(__('{site_title}<br>Your claimed listing <a href="%2$s">%1$s</a> is approved.<br>Thank you.', 'townhub-add-ons'), $listing_post->post_title, get_permalink($listing_post->ID)));
    ...
    public static function lclaim_change_status_to_decline($claim_id = 0)
    {
        if (is_numeric($claim_id) && (int) $claim_id > 0) {
            ...
            $subject = apply_filters('townhub-addons-claim-declined-subject', __('Claim listing declined', 'townhub-add-ons'));
            $body    = apply_filters('townhub-addons-claim-declined-body', sprintf(__('{site_title}<br>Your claimed listing for %s is declined.<br>Thank you.', 'townhub-add-ons'), $listing_post->post_title, get_permalink($listing_post->ID)));
        ...
        if (is_numeric($booking_id) && (int) $booking_id > 0) {
            ...
            if (null != $booking_post) {
                ...
                if (null != $listing_post) {
                    ...
                    if (townhub_addons_get_option('emails_section_auth_booking_insert_admin','yes') == 'yes') $email_recipients[] = get_bloginfo('admin_email');
                    // listing email
        ...
        if (is_numeric($booking_id) && (int) $booking_id > 0) {
            ...
            if (null != $booking_post) {
                ...
                if (null != $listing_post) {
                    ...
                    // also send to site owner
                    if( townhub_addons_get_option('emails_auth_booking_completed_admin','yes') == 'yes' ) $email_recipients[] = get_bloginfo('admin_email');
        ...
        if (is_numeric($booking_id) && (int) $booking_id > 0) {
            ...
            if (null != $booking_post) {
                ...
                if (null != $listing_post) {
                    ...
                    // also send to site owner
                    if( townhub_addons_get_option('emails_section_auth_booking_canceled_admin','yes') == 'yes' ) $email_recipients[] = get_bloginfo('admin_email');
                    // listing email
                    // $email_recipients[] = get_post_meta( $listing_id, ESB_META_PREFIX.'email', true );
                    ...
                    if (townhub_addons_get_option('emails_section_auth_booking_canceled_enable') == 'yes') {
                        ...
                        $booking_insert_auth_temp = apply_filters('listing_booking_insert_auth_temp', townhub_addons_get_option('emails_section_auth_booking_canceled_temp'), $booking_id, $listing_id);
                        $email_template = self::process_email_template($booking_insert_auth_temp, $temp_args);
    ...
    public static function insert_withdrawal($post_id = 0, $author_id = 0)
    {
        if ( !empty($post_id) ) {
            ...
            if (townhub_addons_get_option('emails_auth_new_withdrawal_enable') == 'yes') {
                ...
                $email_subject = self::process_email_template(townhub_addons_get_option('emails_auth_new_withdrawal_subject'), $temp_args);
    ...
    public static function edit_withdrawal_approved($post_id = 0)
    {
        if ( !empty($post_id) ) {
            ...
            if (townhub_addons_get_option('emails_auth_completed_withdrawal_enable') == 'yes') {
                ...
                $email_subject = self::process_email_template(townhub_addons_get_option('emails_auth_completed_withdrawal_subject'), $temp_args);
