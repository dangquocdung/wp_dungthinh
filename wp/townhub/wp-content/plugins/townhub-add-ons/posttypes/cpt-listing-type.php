...
class Esb_Class_Listing_Type_CPT extends Esb_Class_CPT
{
                ...
                <tr class="hoz">
                    ...
                    <td>
                        <select name="price_based">
                            <option value="listing" <?php selected( $price_based, 'listing', true ); ?>><?php _e('Per listing', 'townhub-add-ons') ?></option>
                            <option value="per_person" <?php selected( $price_based, 'per_person', true ); ?>><?php _e('Per person', 'townhub-add-ons') ?></option>
                            <option value="per_night" <?php selected( $price_based, 'per_night', true ); ?>><?php _e('Per night', 'townhub-add-ons') ?></option>
                            <option value="night_person" <?php selected( $price_based, 'night_person', true ); ?>><?php _e('Per person/night', 'townhub-add-ons') ?></option>
                            <option value="per_day" <?php selected( $price_based, 'per_day', true ); ?>><?php _e('Per day', 'townhub-add-ons') ?></option>
                            <option value="day_person" <?php selected( $price_based, 'day_person', true ); ?>><?php _e('Per person/day', 'townhub-add-ons') ?></option>
                            <option value="per_hour" <?php selected( $price_based, 'per_hour', true ); ?>><?php _e('Per hour-slot', 'townhub-add-ons') ?></option>
                            <option value="hour_person" <?php selected( $price_based, 'hour_person', true ); ?>><?php _e('Per person/hour-slot', 'townhub-add-ons') ?></option>
                            
                            <option value="none" <?php selected( $price_based, 'none', true ); ?>><?php _e('No listing price', 'townhub-add-ons') ?></option>
                        </select>
    ...
    public function listing_type_builder_callback($post, $args)
    {
        ...
        wp_localize_script('townhub-react-adminapp', '_townhub_addons_lfields', json_decode($listing_fields));
        wp_localize_script('townhub-react-adminapp', '_townhub_addons_rfields', json_decode($room_fields));
        wp_localize_script('townhub-react-adminapp', '_townhub_addons_frating', json_decode($rating_fields));
        wp_localize_script('townhub-react-adminapp', '_townhub_addons_schema', json_decode($schema_markup));
                        ...
                        if ($term = get_term($fid, 'listing_cat')) {
                            ...
                            if ($child_terms && !is_wp_error($child_terms)) {
                                foreach ($child_terms as $cterm) {
                                    $cterm->name = wp_specialchars_decode( $cterm->name );
                                    $field->cats[] = $cterm;
