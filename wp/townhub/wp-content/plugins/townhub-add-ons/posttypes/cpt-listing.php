...
class Esb_Class_Listing_CPT extends Esb_Class_CPT {     
    ...
    public function submit_listing() {
        ...
        if( isset($_POST['save_as_pending']) && $_POST['save_as_pending'] == 'yes' ){
        ...
        if (!is_wp_error($listing_id)) {
            ...
            if($is_editing_listing == false){
                $lsubscribe = Esb_Class_Membership::current_sub();
                self::update_listing_plan_metas($listing_id, $lsubscribe, $current_member_plan);
            }
    ...
    public static function update_listing_plan_metas($listing_id = 0, $order_id = 0, $plan_id = 0 ){
        ...
        $end_date = Esb_Class_Membership::plan_expire_date($plan_id);
        if($end_date == 'NEVER'){
            update_post_meta( $listing_id, ESB_META_PREFIX.'expire_date',  'NEVER' );
        }else{
    ...
    public static function wkhours($post_id = 0, $for_edit = false){
    ...
    public static function parse_wkhours($post_ID = 0){
        ...
        if( $leventdate != '' ){
            ...
            if( count($eventdates) === 4 && $eventdates[0] != '' ){
                ...
                if( Esb_Class_Date::compare( $start_date, 'now', '>=') ) $leventdate_datas['status'] = 'opening';
            }
        }
        ...
        //     }
        // }
        if( !empty($leventdate_datas) ) return $leventdate_datas;

        // normal type
        $post_working_hours = self::wkhours($post_ID);
