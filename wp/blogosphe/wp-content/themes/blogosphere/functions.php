...
if (!function_exists('blogosphere_system_fonts_list')) {
	function blogosphere_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('blogosphere_text_decoration_list')) {
	function blogosphere_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'blogosphere'), 
			'underline' => esc_html__('underline', 'blogosphere'), 
			'overline' => esc_html__('overline', 'blogosphere'), 
			'line-through' => esc_html__('line-through', 'blogosphere'), 
		);
...
if (!function_exists('blogosphere_custom_color_schemes_list')) {
	function blogosphere_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'blogosphere'), 
			'second' => esc_html__('Custom 2', 'blogosphere'), 
			'third' => esc_html__('Custom 3', 'blogosphere') 
		);
		
		
		return apply_filters('blogosphere_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('blogosphere_locate_template')) {
	function blogosphere_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_blogosphere_theme_style') ? get_option('cmsmasters_blogosphere_theme_style') : ''));
...
if (!function_exists('blogosphere_all_theme_styles')) {
	function blogosphere_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('blogosphere_all_color_schemes_list')) {
	function blogosphere_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'blogosphere'), 
			'header' => 		esc_html__('Header', 'blogosphere'), 
			'navigation' => 	esc_html__('Navigation', 'blogosphere'), 
			'header_top' => 	esc_html__('Header Top', 'blogosphere'), 
			'footer' => 		esc_html__('Footer', 'blogosphere') 
		);
		
		
		$out = array_merge($list, blogosphere_custom_color_schemes_list());
...
if (!function_exists('blogosphere_load_theme_textdomain')) {
	function blogosphere_load_theme_textdomain() {
		load_theme_textdomain('blogosphere', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'blogosphere_load_theme_textdomain')) {
	add_action('after_setup_theme', 'blogosphere_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('blogosphere_theme_activation')) {
	function blogosphere_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'blogosphere') {
			add_option('cmsmasters_active_theme', 'blogosphere', '', 'yes');
...
add_action('after_switch_theme', 'blogosphere_theme_activation');



// Framework Deactivation
if (!function_exists('blogosphere_theme_deactivation')) {
	function blogosphere_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'blogosphere_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('blogosphere_plugin_activation')) {
	function blogosphere_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'blogosphere_plugin_activation', 10, 2);


if (!function_exists('blogosphere_plugin_activation_regenerate')) {
	function blogosphere_plugin_activation_regenerate() {
...
add_action('init', 'blogosphere_plugin_activation_regenerate');


function blogosphere_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = blogosphere_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'blogosphere_run_reinit_import_options', 10, 3);
