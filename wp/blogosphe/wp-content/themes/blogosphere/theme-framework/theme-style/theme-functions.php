...
/* Scripts for Admin */
function blogosphere_theme_admin_scripts() {
	$cmsmasters_option = blogosphere_get_global_options();
	
	wp_enqueue_script('blogosphere-theme-settings-toggle', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/admin/js/theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('blogosphere-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'blogosphere', 
		'header_style' => $cmsmasters_option['blogosphere' . '_header_styles'] 
	));
}

add_action('admin_enqueue_scripts', 'blogosphere_theme_admin_scripts');



/* Register theme JS Scripts */
function blogosphere_register_theme_js_scripts() {
	$cmsmasters_option = blogosphere_get_global_options();
	...
	wp_localize_script('blogosphere-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'blogosphere_register_theme_js_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'blogosphere'), 
    'footer' => 	esc_html__('Footer Navigation', 'blogosphere'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'blogosphere') 
));
...
function blogosphere_fallback_menu($args) {
	$cmsmasters_option = blogosphere_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['blogosphere' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function blogosphere_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'blogosphere_single_project_layout');



/* Search Form */
function blogosphere_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('Search', 'blogosphere') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'blogosphere_search_form');



/* Pagination Prev Text */
function blogosphere_pagination_prev_text() {
	return esc_html__('previous', 'blogosphere');
}

add_filter('blogosphere_pagination_prev_text_filter', 'blogosphere_pagination_prev_text');



/* Pagination Next Text */
function blogosphere_pagination_next_text() {
	return esc_html__('next', 'blogosphere');
}

add_filter('blogosphere_pagination_next_text_filter', 'blogosphere_pagination_next_text');
