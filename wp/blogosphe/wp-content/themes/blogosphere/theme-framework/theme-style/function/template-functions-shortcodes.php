...
function blogosphere_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true, $link_target = false) { 
	if ($type == 'post') {
...
function blogosphere_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function blogosphere_slider_post_check_exc_cont($type = 'post') {
	$exc = blogosphere_slider_post_exc_cont($type, false);
...
function blogosphere_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function blogosphere_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsmasters_slider_post_author">' . 
			esc_html__('By', 'blogosphere') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'blogosphere') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
...
function blogosphere_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function blogosphere_slider_post_views($type = 'post', $show = true) {
	$out = '';
...
/* Get Posts Slider Like Function */
function blogosphere_slider_post_like($type = 'post', $show = true) {
	$out = '';
	
	
	if ($type == 'post') {
		$out = cmsmasters_like('cmsmasters_slider_post_likes', '', 'true');
	} elseif ($type == 'project') {
		$out = cmsmasters_like('cmsmasters_slider_project_likes', '', 'true');
	}
...
function blogosphere_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = blogosphere_get_comments('cmsmasters_slider_post_comments', '', true);
		} elseif ($type == 'project') {
			$out = blogosphere_get_comments('cmsmasters_slider_project_comments', '', true);
		}
	}
...
function blogosphere_slider_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_attr__('Read More', 'blogosphere');
	}
