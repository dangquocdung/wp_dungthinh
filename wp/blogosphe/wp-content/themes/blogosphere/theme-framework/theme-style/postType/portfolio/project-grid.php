...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && blogosphere_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		if ($categories || $title || $excerpt || $comments || $likes || $views) {
			echo '<div class="project_inner">';
				...
				if ($categories) {
					echo '<div class="cmsmasters_project_cont_info entry-meta">';
						...
						blogosphere_get_project_category(get_the_ID(), 'pj-categs', 'page');
						
					echo '</div>';
				}
				
				
				$title ? blogosphere_project_heading(get_the_ID(), 'h4', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
			
			
				blogosphere_thumb_rollover(get_the_ID(), $project_thumb_size, false, $rollover, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
				
				
				$excerpt ? blogosphere_project_exc_cont() : '';
				
				
				if ($comments || $likes || $views) {
					echo '<footer class="cmsmasters_project_footer entry-meta">';
						
						$comments ? blogosphere_get_project_comments('page') : '';
						
						$likes ? blogosphere_get_project_likes('page') : '';
						
						$views ? blogosphere_get_project_views('page') : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				blogosphere_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
