...
function weberium_breadcrumbs() {

    global $post;

    $home_text     = esc_html__('Home','weberium');
    $category_text = esc_html__('Archive by Category "%s"','weberium');
    $tax_text      = esc_html__('Archive by "%s"','weberium');
    $tag_text      = esc_html__('Posts Tagged "%s"','weberium');
    $author_text   = esc_html__('Articles Posted by %s','weberium');
    $error_text    = esc_html__('Error 404','weberium');
    $search_text   = esc_html__('Search Results for "%s" Query','weberium');

    $blog_text   = esc_html__('Blog','weberium');
	$gallery_text = esc_html__('Galleries','weberium');
	$product_text = esc_html__('Products','weberium');
    ...
    if ( get_query_var('paged') ) {
        ...
        echo '<span>'. esc_html__( 'Page','weberium' ) .' '. get_query_var('paged') .'</span>';

        if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) { echo ')'; }
    }
} // End weberium_breadcrumbs()