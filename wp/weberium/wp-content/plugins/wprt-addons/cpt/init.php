...
if ( ! class_exists( 'WPRT_CPT' ) ) {
	class WPRT_CPT {
		function __construct() {
			...
			add_filter( 'single_template', array( $this,'weberium_single_project' ) );	    
			add_filter( 'archive_template', array( $this,'weberium_archive_project' ) );	    
	    }

		function weberium_single_project( $single_template ) {
			global $post;
			if ( $post->post_type == 'project' ) $single_template = dirname( __FILE__ ) . '/inc/single-project.php';
			return $single_template;
		}

		function weberium_archive_project( $archive_template ) {
			global $post;
