...
<div class="weberium-team <?php echo esc_attr( $cls ); ?>" data-auto="<?php echo esc_attr( $auto_scroll ); ?>" data-column="<?php echo esc_attr( $column ); ?>" data-column2="<?php echo esc_attr( $column2 ); ?>" data-column3="<?php echo esc_attr( $column3 ); ?>" data-gap="<?php echo esc_html( $gap ); ?>">
	    ...
	    <?php while ( $query->have_posts() ) : $query->the_post(); ?>
        	<div class="team-item clearfix">
        		<div class="inner">
		        	...
		        	if ( weberium_metabox( 'facebook' )
        			|| weberium_metabox( 'twitter' )
        			|| weberium_metabox( 'linkedin' )
        			|| weberium_metabox( 'google_plus' )
        			|| weberium_metabox( 'instagram' ) ) :

		        	$socials_html .= '<ul class="socials clearfix">';
		        	if ( weberium_metabox( 'facebook' ) )
	        		$socials_html .= '<li class="facebook"><a target="_blank" href="'. esc_url( weberium_metabox( 'facebook' ) ) .'"><i class="nz-facebook"></i></a></li>';

	        		if ( weberium_metabox( 'twitter' ) )
	        		$socials_html .= '<li class="twitter"><a target="_blank" href="'. esc_url( weberium_metabox( 'twitter' ) ) .'"><i class="nz-twitter"></i></a></li>';

	        		if ( weberium_metabox( 'linkedin' ) )
	        		$socials_html .= '<li class="linkedin"><a target="_blank" href="'. esc_url( weberium_metabox( 'linkedin' ) ) .'"><i class="nz-linkedin"></i></a></li>';

	        		if ( weberium_metabox( 'google_plus' ) )
	        		$socials_html .= '<li class="google-plus"><a target="_blank" href="'. esc_url( weberium_metabox( 'google_plus' ) ) .'"><i class="nz-google-plus"></i></a></li>';

	        		if ( weberium_metabox( 'instagram' ) )
	        		$socials_html .= '<li class="instagram"><a target="_blank" href="'. esc_url( weberium_metabox( 'instagram' ) ) .'"><i class="nz-instagram"></i></a></li>';
		        	$socials_html .= '</ul>';
		            ...
		            <?php if ( has_post_thumbnail() ) : ?>
                 	...
                 	<?php if ( weberium_metabox( 'name' ) || weberium_metabox( 'position' ) || weberium_metabox( 'text' ) ) : ?>
                 		<div class="text-wrap" style="<?php echo esc_attr($content_css); ?>">
					        <?php if ( weberium_metabox( 'name' ) ) : ?>
					        	<h4 class="name" style="<?php echo esc_attr( $name_css ); ?>">
					        		<span><?php echo esc_html( weberium_metabox( 'name' ) ); ?></span></h4>
					        <?php endif; ?>

					        <?php if ( weberium_metabox( 'position' ) ) : ?>
					        	<div class="position" style="<?php echo esc_attr( $position_css ); ?>">
					        	<?php echo esc_html( weberium_metabox( 'position' ) ); ?></div>
					        <?php endif; ?>
				        </div>
			        <?php endif; ?>
        		</div><!-- /.inner -->
