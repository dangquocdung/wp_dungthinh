...
extract( shortcode_atts( array(
    ...
    'line_bg_color' => '#b9e9e9',
    'line_bg_height' => '13px',
    'heading_padding' => '0px 5% 3px 5%',
...
$line_bg_height = intval( $line_bg_height );

$css = $image_css = $heading_css = $heading_bg_css = $sep_css = $desc_css = $button_cls = $button_css = $inner_css = $thumb_css = $content_css = '';
$image_html = $heading_html = $sep_html = $desc_html = $button_html = '';
$heading_cls = $heading_style;
...
if ( $heading_font_family != 'Default' ) {
    weberium_enqueue_google_font( $heading_font_family );
...
if ( $desc_font_family != 'Default' ) {
    weberium_enqueue_google_font( $desc_font_family );
...
if ( $button_font_family != 'Default' ) {
    weberium_enqueue_google_font( $button_font_family );
...
if ( $heading_style == 'style-2' ) {
    if ( $heading_padding ) $heading_bg_css .= 'padding:'. $heading_padding .';';

    if ( $line_bg_color ) {
        $heading_bg_css .= 'background-image: -webkit-gradient(linear, left top, right top, from('. $line_bg_color .'), to('. $line_bg_color .'));';
        $heading_bg_css .= 'background-image: linear-gradient(to right,'. $line_bg_color .' 0%, '. $line_bg_color .' 100%);';
    }

    if ( $line_bg_height )
        $heading_bg_css .= 'background-size: 100% '. $line_bg_height .'px;';
}

if ( $heading )
    $heading_html .= sprintf(
    '<%5$s class="title %6$s" style="%2$s">
        <a target="%4$s" href="%3$s"><span style="%7$s">%1$s</span></a>
    </%5$s>',
    esc_html( $heading ),
    $heading_css,
    esc_attr( $link_url ),
...
if ( $button_text ) {
    $button_cls = $button_size .' '. $button_padding .' '. $border_style;
    if ( $button_style == 'simple_link' ) $button_cls .= ' simple-link font-heading';
    if ( $button_style == 'accent' ) $button_cls .= ' weberium-button accent';
    if ( $button_style == 'dark' ) $button_cls .= ' weberium-button dark';
    if ( $button_style == 'light' ) $button_cls .= ' weberium-button light';
    if ( $button_style == 'very-light' ) $button_cls .= ' weberium-button very-light';
    if ( $button_style == 'white' ) $button_cls .= ' weberium-button white';
    if ( $button_style == 'outline' ) $button_cls .= ' weberium-button outline ol-accent';
    if ( $button_style == 'outline_dark' ) $button_cls .= ' weberium-button outline dark';
    if ( $button_style == 'outline_light' ) $button_cls .= ' weberium-button outline light';
    if ( $button_style == 'outline_very-light' ) $button_cls .= ' weberium-button outline very-light';
    if ( $button_style == 'outline_white' ) $button_cls .= '  weberium-button outline white';
...
printf(
    '<div class="weberium-image-box clearfix %7$s" style="%8$s">
        <div class="item">
