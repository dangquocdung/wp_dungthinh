...
if ( $price_font_family != 'Default' ) {
	weberium_enqueue_google_font( $price_font_family );
...
if ( $unit_font_family != 'Default' ) {
	weberium_enqueue_google_font( $unit_font_family );
...
if ( $price || $price_unit )
	$p_html .= sprintf(
		'<div class="price-table-price %4$s" style="%3$s">
			<div class="price-wrap"><span>%1$s %2$s</span></div>
		</div>',
		$p1_html,
		$p2_html,
		$price_css,
...
if ( $button_text ) {
	$button_cls = $button_size .' '. $btn_padding .' '. $border_style;
...
printf(
	'<div class="weberium-price-table %1$s" style="%2$s">
		%5$s %6$s
