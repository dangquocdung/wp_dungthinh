 ...
 * by `AArpr_scss_parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `AArpr_scssc`. The CSS tree is fed into a
 * formatter, like `AArpr_scss_formatter` which then outputs CSS as a string.
 *
 * During the first compile, all values are *reduced*, which means that their
 * types are brought to the lowest form before being dump as strings. This
 * handles math equations, variable dereferences, and the like.
 *
 * The `parse` function of `AArpr_scssc` is the entry point.
 *
 * In summary:
 *
 * The `AArpr_scssc` class creates an instance of the parser, feeds it SCSS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `AArpr_scss_parser` class is only concerned with parsing its input.
 *
 * The `AArpr_scss_formatter` takes a CSS tree, and dumps it to a formatted string,
...
class AArpr_scssc {
	...
	protected $formatter = "AArpr_scss_formatter_nested";
	...
	public function compile($code, $name = null)
	{
		...
		$this->parser = new AArpr_scss_parser($name);
	...
	protected function injectVariables(array $args)
	{
		...
		$parser = new AArpr_scss_parser(__METHOD__, false);
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
			...
			$parser = new AArpr_scss_parser($path, false);
...
class AArpr_scss_parser {
	...
	static protected function makeOperatorStr($operators) {
		return '('.implode('|', array_map(array('AArpr_scss_parser','preg_quote'),
			$operators)).')';
	}
	 ...
	 *
	 * Consider the function AArpr_scssc::keyword(). (All parse functions are
	 * structured the same.)
	 *
	 ...
	 *
	 * All of these parse functions are powered by AArpr_scssc::match(), which behaves
	 ...
	 * avoid this, AArpr_scssc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function selectorSingle(&$out) {
		...
		while (true) {
			...
			// self
			if ($this->literal("&", false)) {
				$parts[] = AArpr_scssc::$selfSelector;
				continue;
			}
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class AArpr_scss_formatter_nested extends AArpr_scss_formatter {
	public $close = " }";
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class AArpr_scss_formatter_compressed extends AArpr_scss_formatter {
	public $open = "{";
	public $tagSeparator = ",";
	public $assignSeparator = ":";
	public $break = "";
...
class AArpr_scss_server {
	 ...
	 * @param \AArpr_scssc|null $scss     SCSS compiler instance
	 */
