...
class Repeater extends Field_Abstract {
	...
	public function render_template() {
		?>
		<script type="text/html" id="tmpl-wvt-field-repeater">
			<?php $this->js_template(); ?>
		</script>
		<script type="text/html" id="tmpl-wvt-field-repeater-content">
		...
		<# var field; var index = data.index; #>
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, fieldID ) { #>
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									...
									<# if ( field.description ) { #>
										<span class="description customize-control-description">{{{ field.description }}}</span>
									<# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'wvt' ); ?>"  value="{{{ field.default }}}" data-field="{{{ fieldID }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="jeg-image-attachment" data-placeholder="<?php echo esc_attr__( 'No Image Selected', 'wvt' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php echo esc_attr__( 'No Image Selected', 'wvt' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr__( 'Remove', 'wvt' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr__( 'Add Image', 'wvt' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change Image', 'wvt' ); ?>" >
										<# if ( field.default ) { #>
											<?php echo esc_attr__( 'Change Image', 'wvt' ); ?>
										<# } else { #>
											<?php echo esc_attr__( 'Add Image', 'wvt' ); ?>
										<# } #>
									</button>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="jeg-file-attachment" data-placeholder="<?php echo esc_attr__( 'No File Selected', 'wvt' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php echo esc_attr__( 'No File Selected', 'wvt' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr__( 'Remove', 'wvt' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php echo esc_attr__( 'Add File', 'wvt' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change File', 'wvt' ); ?>" >
										<# if ( field.default ) { #>
											<?php echo esc_attr__( 'Change File', 'wvt' ); ?>
										<# } else { #>
											<?php echo esc_attr__( 'Add File', 'wvt' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php echo esc_attr__( 'delete', 'wvt' ); ?></button>
				</div>
			</li>
		<?php
	}
