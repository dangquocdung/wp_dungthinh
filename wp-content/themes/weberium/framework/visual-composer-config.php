...
class Weberium_VC_Config {

	// Start things up
	public function __construct() {
		// Check if design options are enabled
		$remove_design_options = apply_filters( 'weberium_remove_vc_design_options', true );
		...
		if ( is_admin() ) {
			...
			add_action( 'admin_notices', array( 'Weberium_VC_Config', 'vc_license_tab_notice' ) );
		}

		// Run on init
		add_action( 'init', array( 'Weberium_VC_Config', 'init' ), 20 );

		// Tweak scripts
		add_action( 'wp_enqueue_scripts', array( 'Weberium_VC_Config', 'load_composer_front_css' ), 0 );
		add_action( 'wp_enqueue_scripts', array( 'Weberium_VC_Config', 'load_remove_styles' ) );
		add_action( 'wp_footer', array( 'Weberium_VC_Config', 'remove_footer_scripts' ) );

		// Remove VC welcome screen
		add_action( 'admin_menu', array( 'Weberium_VC_Config', 'remove_welcome' ), 999 );
	...
	public static function vc_license_tab_notice() {
		...
		if ( 'visual-composer_page_vc-updater' == $screen->id )
			echo '<div class="error"><p><strong>'. esc_html__( 'Activating the Visual Composer plugin is 100% optional and NOT required to function correctly with the theme.', 'weberium' ) .'</strong></p></div>';
	}
	...
	public static function load_remove_styles() {
		...
		if ( apply_filters( 'weberium_remove_vc_design_options', true ) )
